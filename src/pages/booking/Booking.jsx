...
useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        const bookingsData = response.filter((item) => item.warehouse === localStorage.getItem("warehouse"));

        let filteredBookings = statusFilter === "All"
          ? bookingsData
        ...
        if (timePeriod === "last7Days") {
          ...
          filteredBookings = filteredBookings.filter(booking => new Date(booking.companyBargainDate) >= filterDate);
        ...
        filteredBookings.sort((a, b) => new Date(b.companyBargainDate) - new Date(a.companyBargainDate));
  ...
  const handleTransferSubmit = async (booking) => {
    ...
    try {
      ...
      const itemsToUpdate = booking.items.map(item => {
        const currentItem = latestbooking?.items?.find(i => i.name === item.name);
  ...
  const handleDownloadExcel = () => {
    const formattedbookings = bookings.map(booking => ({
      ...
      'Delivery Location': booking.deliveryAddress?.addressLine1 + ", " +  booking.deliveryAddress?.addressLine2 + ", " +  booking.deliveryAddress?.city + ", " +  booking.deliveryAddress?.state + ", " +  booking.deliveryAddress?.pinCode,
    ...
    {!showBookingForm ? (
      <Card>
        ...
        >
          ...
          <Button variant="gradient" color="blue" onClick={handleCreateBookingClick}>
            Create Booking
          </Button>
        </CardHeader>
          ...
          {loading ? (
            ...
            bookings.length > 0 ? (
              <table className="w-full min-w-[640px] table-auto">
                <thead>
                  <tr>
                    {["Company Bargain Date", "Company Bargain No", "Buyer Name", "Buyer Location", "Buyer Contact", "Status", "Delivery Type", "Delivery Location", "Actions"].map((el) => (
                      <th
                        key={el}
                        ...
                        <tr>
                          ...
                          <td className={className}>
                            <Chip
                              ...
                              color={booking.status === "created" ? "blue" : booking.status === "payment pending" ? "yellow" : booking.status === "billed" ? "green" : "red"}
                            />
                          </td>
                          <td className={className}>
                            <Typography className="text-xs font-semibold text-center text-blue-gray-600">
                          ...
                          <td className={className}>
                            <Typography className="text-xs font-semibold text-center text-blue-gray-600">
                              {booking.deliveryAddress?.addressLine1}, {booking.deliveryAddress?.addressLine2}, {booking.deliveryAddress?.city}, {booking.deliveryAddress?.state}, {booking.deliveryAddress?.pinCode}
                            </Typography>
                          </td>
                          <td className={className}>
                              ...
                              <td colSpan="11">
                                <div className="p-4 bbooking-t bbooking-blue-gray-200">
                                  ...
                                  <table className="w-full table-auto">
                                    <thead>
                                      <tr>
                                        {["Item Name", "Packaging", "Weight", "Static Price (Rs.)", "Virtual Quantity", "Billed Quantity"].map((header) => (
                                          <th key={header} className="bbooking-b bbooking-blue-gray-50 py-3 px-5 text-left">
                                    ...
                                    <tbody>
                                      {booking.items.map((item) => (
                                        <tr key={item.name}>
                                          <td className="bbooking-b bbooking-blue-gray-50 py-3 px-5 text-center text-blue-gray-600">{item.name}</td>
                                          <td className="bbooking-b bbooking-blue-gray-50 py-3 px-5 text-center text-blue-gray-600">{item.packaging}</td>
                                          <td className="bbooking-b bbooking-blue-gray-50 py-3 px-5 text-center text-blue-gray-600">{item.weight}</td>
                                          <td className="bbooking-b bbooking-blue-gray-50 py-3 px-5 text-center text-blue-gray-600">{item.staticPrice}</td>
                                          <td className="bbooking-b bbooking-blue-gray-50 py-3 px-5 text-center text-blue-gray-600">{item.quantity}</td>
                                          <td className="bbooking-b bbooking-blue-gray-50 py-3 px-5 text-center text-blue-gray-600">{item.billedQuantity ? item.billedQuantity : "0"}</td>
              ...
              <Typography className="text-center text-blue-gray-600">No bookings found</Typography>
            ))}
          </CardBody>
        </Card>
      ) : (
        <CreateBookingForm setShowCreateBookingForm={setBookingForm} close={() => setBookingForm(false)} />
      )}
